...
BEGIN
   ...
   SELECT @ImageData = (
         SELECT convert (VARBINARY (max), thumbnailphoto, 1)
         FROM AdventureWorks2019.Production.Productphoto
         WHERE productphotoid= @PicName
         );
...
my $sth1 = $dbh->prepare ("{call usp_ExportImage5(?, ?, ?)}");

$sth1 -> execute($myid,'D:\skill tracking\perl\programs\testdir',$myid.test); 
